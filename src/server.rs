...
impl Inventory for StoreInventory {
    async fn add(
        ...
        // validate stock, verify it's present and price is not negative or $0.00
    ...
    async fn update_quantity(&self, request: Request<QuantityChangeRequest>) -> Result<Response<InventoryUpdateResponse>, Status> {
        ...
        // check if quantity change is zero first
        if identifier.change == 0 {
            return Err(Status::invalid_argument(EMPTY_QUANT_ERR));
        }

        // make sure item exists
        let mut  map = self.inventory.lock().await;
        let item = match map.get_mut(&identifier.sku) {
            Some(item) => item,
            None => return Err(Status::not_found(NO_ITEM_ERR)),
        };
        ...
        stock.quantity  = match identifier.change {
            // handle reduction
            change if change < 0 => {
    ...
    async fn update_price(&self, request: Request<PriceChangeRequest>) -> Result<Response<InventoryUpdateResponse>, Status> {
        let identifier = request.into_inner();
